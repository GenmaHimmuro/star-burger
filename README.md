...
Определите переменные окружения `SECRET_KEY` и `YANDEX_API_KEY`. Для получения API-ключа перейдите на [Яндекс.Девелопер](https://developer.tech.yandex.ru/services). Создать файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
...
- `YANDEX_API_KEY` = API-ключ [Яндекс.Девелопер](https://developer.tech.yandex.ru/services). Сервис для получения координат по адресу.
